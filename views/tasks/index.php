...
/* ===== TABS ===== */
.view-switcher { background: #e2e8f0; padding: 0.3rem; border-radius: 0.8rem; display: inline-flex; }
...
.column-header h6 { margin: 0; font-weight: 800; font-size: 0.72rem; text-transform: uppercase; color: #475569; }
...
.task-card:hover { transform: translateY(-1px); box-shadow: 0 3px 8px rgba(0,0,0,0.07); border-color: #6366f1; }
.task-card.rejected { background: #fff1f2; border-color: #fecaca; }
.task-card.extended { background: #fefce8; border-color: #fde68a; }
.task-card.locked-card { cursor: default; opacity: 0.7; user-select: none; }
.task-card.locked-card:hover { transform: none; box-shadow: none; border-color: #e2e8f0; }

.card-label { font-size: 0.58rem; font-weight: 800; color: #6366f1; text-transform: uppercase; margin-bottom: 0.15rem; }
.card-name { font-weight: 700; font-size: 0.82rem; color: #1e293b; margin-bottom: 0.3rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
.card-desc-preview { font-size: 0.68rem; color: #94a3b8; margin-bottom: 0.3rem; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden; }
.card-info { display: flex; justify-content: space-between; align-items: center; padding-top: 0.4rem; border-top: 1px solid #f1f5f9; gap: 0.2rem; }
.assignee { font-size: 0.62rem; color: #64748b; font-weight: 600; }
.priority-badge { font-size: 0.52rem; font-weight: 800; padding: 0.12rem 0.4rem; border-radius: 0.3rem; text-transform: uppercase; }
.priority-high { background: #fee2e2; color: #dc2626; }
.priority-medium { background: #fef3c7; color: #d97706; }
.priority-low { background: #dcfce7; color: #16a34a; }
.deadline-text { font-size: 0.58rem; color: #94a3b8; }
.deadline-overdue { color: #dc2626 !important; font-weight: 700; }
...
/* PROGRESS BAR */
.task-progress { padding: 0.35rem 0.85rem 0.4rem; background: #f8fafc; flex-shrink: 0; }
.progress-bar-wrap { height: 5px; background: #e2e8f0; border-radius: 3px; overflow: hidden; }
.progress-bar-fill { height: 100%; border-radius: 3px; background: linear-gradient(90deg, #6366f1, #8b5cf6); transition: width 0.4s; }
.progress-bar-fill.complete { background: linear-gradient(90deg, #10b981, #34d399); }
.progress-text { font-size: 0.58rem; color: #94a3b8; font-weight: 700; margin-top: 0.15rem; text-align: right; }

/* Task hoàn thành */
.task-completed-column { border: 2px solid #10b981 !important; background: #f0fdf4 !important; }
.task-completed-column .column-header { background: #f0fdf4; }

/* SUBTASK LIST MODAL */
.subtask-list-item { padding: 0.45rem 0.65rem; border-bottom: 1px solid #f1f5f9; font-size: 0.8rem; display: flex; justify-content: space-between; align-items: center; cursor: pointer; transition: background 0.15s; border-left: 3px solid transparent; }
...
/* DYNAMIC ROWS */
.subtask-row { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 0.6rem; padding: 0.6rem; margin-bottom: 0.4rem; position: relative; }
.subtask-row .remove-row { position: absolute; top: 0.3rem; right: 0.4rem; color: #ef4444; cursor: pointer; font-size: 0.9rem; }
    ...
    <div class="board-container">
                    ...
                    <div class="task-card <?php echo (($c['is_rejected'] ?? 0) && ($c['status'] ?? '') == 'To Do') ? 'rejected' : ''; ?> <?php echo $isExtended ? 'extended' : ''; ?> <?php echo $isDone ? 'locked-card' : ''; ?>"
                        ...
                        <div class="card-info">
                            ...
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <?php endif; ?>
                                <?php
                                    $pClass = 'priority-medium'; $pLabel = 'TB';
        ...
        <div id="board-tasks" class="board-tasks d-none">
            <?php foreach ($tasksWithSubtasks as $task):
                ...
                $dCount = $task['done_count'] ?? count(array_filter($task['subtasks'], function($s) { return $s['status'] == 'Done'; }));
                    ...
                    <div class="task-card <?php echo (($c['is_rejected'] ?? 0) && ($c['status'] ?? '') == 'To Do') ? 'rejected' : ''; ?> <?php echo $isExtended ? 'extended' : ''; ?>"
                        ...
                        <div class="card-info">
                            ...
                            <div class="d-flex align-items-center gap-1">
                                ...
                                ?>
                                <span class="badge <?php echo $stClass; ?>" style="font-size: 0.52rem;"><?php echo $isCardOverdue ? 'Trễ hạn' : $c['status']; ?></span>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; ?>
                    <?php if($_SESSION['role_id'] != 3): ?>
                    <button class="btn btn-link btn-sm w-100 text-muted mt-1 fw-bold text-decoration-none" onclick="openAddSubtaskModal(<?php echo $task['id']; ?>)">+ Thêm việc...</button>
                    <?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 1rem;">
            ...
            <div class="modal-body">
                <form id="formTask">
                    <div class="mb-3"><label class="form-label small fw-bold">TIÊU ĐỀ TASK <span class="text-danger">*</span></label><input type="text" name="title" class="form-control rounded-3" required></div>
                    <div class="mb-3"><label class="form-label small fw-bold">MÔ TẢ <span class="text-danger">*</span></label><textarea name="description" class="form-control rounded-3" rows="2" required></textarea></div>
                    <div class="row mb-3">
                        ...
                        <div class="col-6"><label class="form-label small fw-bold">HẠN CHÓT</label><input type="date" name="deadline" class="form-control rounded-3 date-no-past"></div>
                    </div>
                    <hr>
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <label class="form-label small fw-bold mb-0">CÔNG VIỆC CON <span class="text-danger">* (ít nhất 1)</span></label>
                        <button type="button" class="btn btn-outline-primary btn-sm rounded-pill" onclick="addSubtaskRow('taskSubtaskList')"><i class="bi bi-plus-lg me-1"></i>Thêm</button>
                    </div>
...
<div class="modal fade" id="addSubtaskModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 1rem;">
            ...
            <div class="modal-footer border-0"><button class="btn btn-primary w-100 rounded-pill fw-bold" onclick="saveMultipleSubtasks()">GIAO VIỆC</button></div>
        </div>
    </div>
</div>
...
function saveTask() {
    ...
    $.ajax({ url: 'index.php?action=api_create_task', type: 'POST', data: fd, processData: false, contentType: false, dataType: 'json',
...
function saveMultipleSubtasks() {
    ...
    $.ajax({ url: 'index.php?action=api_create_subtask', type: 'POST', data: fd, processData: false, contentType: false, dataType: 'json',
        success: function(res) {
            if(res.success) Swal.fire({title:'Thành công!', text:res.message, icon:'success', timer:1500, showConfirmButton:false}).then(()=>location.reload());
            else Swal.fire({title:'Lỗi!', text:res.message, icon:'error'});
        }
...
function submitEv(e, id) {
    ...
    $.ajax({ url:'index.php?action=api_submit_evidence', type:'POST', data:fd, processData:false, contentType:false, dataType:'json',
        success: function(res) {
            if(res.success) Swal.fire({title:'Thành công!', text:res.message, icon:'success', timer:1500, showConfirmButton:false}).then(()=>location.reload());
            else Swal.fire({title:'Lỗi!', text:res.message, icon:'error'});
        }
...
function reject(id) {
    ...
    setTimeout(function() {
        Swal.fire({title:'Từ chối công việc', input:'textarea', inputLabel:'Lý do từ chối:', inputPlaceholder:'Nhập lý do...', showCancelButton:true, confirmButtonColor:'#ef4444', confirmButtonText:'Từ chối', cancelButtonText:'Hủy',
...
function saveEvidenceOnly(id) {
    ...
    let fd = new FormData(document.getElementById('evidForm'));
    fd.append('subtask_id', id);
    $.ajax({ url:'index.php?action=api_save_evidence', type:'POST', data:fd, processData:false, contentType:false, dataType:'json',
        success: function(res) {
            if(res.success) {
